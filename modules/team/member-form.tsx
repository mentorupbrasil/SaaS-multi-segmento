...
import { inviteMember, type FormResult } from "./actions";
import { SubmitButton } from "@/components/submit-button";
import { Icon } from "@/components/icon";

const initial: FormResult = {};

export function MemberForm({ professionalLabel }: { professionalLabel: string }) {
  ...
  if (!open) {
    return (
      <button className="btn-primary" onClick={() => setOpen(true)}>
        ...
        Adicionar {professionalLabel.toLowerCase()}
      </button>
    );
  }
  ...
  return (
    <div className="card mb-6 p-6">
      <h2 className="mb-4 text-lg font-semibold">Adicionar {professionalLabel.toLowerCase()}</h2>

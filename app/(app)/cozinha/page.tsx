...
const COLUMNS = [
  { status: "PENDING" as const, title: "Pendente", headerClass: "bg-muted text-foreground" },
        ...
        {COLUMNS.map((col) => {
          ...
          return (
            <section key={col.status} className="flex flex-col">
              ...
              >
                {col.title}
                <span className="ml-2 rounded-full bg-card/60 px-2 py-0.5 text-xs">
                  {columnOrders.length}
                </span>
              </div>
              <div className="flex flex-1 flex-col gap-3">
                {columnOrders.length === 0 ? (
                  <div className="card flex flex-1 items-center justify-center p-6 text-center text-sm text-muted-foreground">
                    Nenhum pedido
                  </div>
                ) : (
                  columnOrders.map((order) => {
                    ...
                    return (
                      <div key={order.id} className="card p-4">
                        ...
                        {order.notes && (
                          <p className="mb-3 rounded-lg bg-muted px-2 py-1 text-xs text-muted-foreground">
                            {order.notes}
                          </p>
                        )}

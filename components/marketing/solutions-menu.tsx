...
function SolutionLink({ solution, onClose }: { solution: Solution; onClose?: () => void }) {
  return (
    <Link
    ...
    >
      ...
      <span className="min-w-0 flex-1">
        <span className="block text-sm font-semibold text-foreground group-hover:text-primary">{solution.title}</span>
        <span className="mt-0.5 block text-xs font-medium text-muted-foreground">{solution.headline}</span>
      </span>
...
export function SolutionsMenuPanel({ onClose }: { onClose?: () => void }) {
  ...
  const searchResults = useMemo(
    () => (isSearching ? filterSolutions(query).slice(0, 10) : []),
    [isSearching, query],
  );
    ...
    <div className="w-[min(820px,calc(100vw-2rem))]">
      <div className="border-b border-border px-5 pb-4 pt-4">
        ...
        <p className="mt-0.5 text-xs text-muted-foreground">
          Financeiro, clientes, vendas, automação… · {total} soluções
        </p>
      ...
      {isSearching ? (
        <div className="p-5">
            ...
            <ul className="grid gap-1 sm:grid-cols-2">
              {searchResults.map((solution) => (
                <li key={solution.slug}>
                  <SolutionLink solution={solution} onClose={onClose} />
                </li>
              ))}
            </ul>
          )}
          <p className="mt-4 border-t border-border pt-4 text-center">
            <Link
              href={`/solucoes?q=${encodeURIComponent(query.trim())}`}
              onClick={onClose}
              className="text-sm font-semibold text-primary hover:underline"
            >
              Ver todas na página de soluções →
            </Link>
          </p>
        </div>
      ) : (
            ...
            <div key={group.id} className="bg-muted/20 p-4">
              ...
              <ul className="space-y-0.5">
                {group.items.map((solution) => (
                  <li key={solution.slug}>
                    <HoveredLink href={`/solucoes#${solution.slug}`} onClick={onClose}>
                      {solution.title}
                    </HoveredLink>
                  </li>
                ))}
      ...
      <div className="border-t border-border px-5 py-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <Link href="/solucoes" onClick={onClose} className="shrink-0 text-xs font-semibold text-primary hover:underline">
            Ver catálogo ({total}) →
          </Link>
        </div>
      </div>

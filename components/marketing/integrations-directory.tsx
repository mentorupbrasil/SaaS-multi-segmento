...
function IntegrationCard({ item }: { item: Integration }) {
  return (
    <article className="group relative flex flex-col overflow-hidden rounded-2xl border border-border bg-card p-6 shadow-sm transition-all hover:-translate-y-0.5 hover:border-primary/30 hover:shadow-lg hover:shadow-brand-500/5">
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2">
        <h3 className="text-lg font-semibold text-foreground group-hover:text-primary">{item.name}</h3>
      ...
      <div className="mt-5 flex items-center justify-between gap-3 border-t border-border pt-4">
        <span className="text-xs font-medium text-muted-foreground">
          Plano: <span className="font-semibold text-primary">{item.planLabel}</span>
        </span>
        <Link
          href="/signup"
...
export function IntegrationsDirectory({ initialQuery = "" }: IntegrationsDirectoryProps) {
  ...
  return (
    <div className="section pb-16">
      ...
      <div className="grid gap-4 sm:grid-cols-[1fr_auto] sm:items-center">
        ...
        <div className="relative w-full sm:max-w-xs">
          ...
          <input
            ...
            className="w-full rounded-xl border border-border bg-card py-2.5 pl-10 pr-4 text-sm text-foreground shadow-sm outline-none ring-brand-500 transition-shadow placeholder:text-muted-foreground focus:border-primary/50 focus:ring-2"
          />
        </div>
      </div>
      ...
      {showFeatured && (
        <div className="mt-10">
          <p className="text-xs font-semibold uppercase tracking-wider text-muted-foreground">Mais usadas</p>
      ...
      <div className="mt-10 flex flex-wrap gap-2">
        <button
          ...
          className={cn(
            ...
            activeGroup === "all"
              ? "bg-primary text-primary-foreground shadow-md shadow-brand-600/20"
      ...
      <div className="mt-8">
        {filtered.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border bg-muted/30 px-6 py-16 text-center">
            ...
            <p className="mt-1 text-sm text-muted-foreground">Tente outro termo ou limpe os filtros.</p>
            <button
              type="button"
              onClick={() => {
                setQuery("");
